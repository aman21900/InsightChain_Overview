...
from chains import (build_decomposer_chain, build_sql_chain, build_semantic_chain, build_sufficiency_chain,
                    build_reducer_chain, build_sql_repair_chain)
...
def ensure_chains():
    global _decomposer, _sql_chain, _semantic, _sufficiency, _reducer, _sql_repair
    print(f"[ENSURE_CHAINS] Called. _decomposer={_decomposer}, _sql_chain={_sql_chain}")  # add this
    if any(c is None for c in [_decomposer, _sql_chain, _semantic, _sufficiency, _reducer, _sql_repair]):
